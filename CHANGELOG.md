...
- Generator cleanup: previously generated files are tracked in a manifest
  (`.dart_tool/flutter_assistant_intents/outputs.json`); when output paths
  change, stale files are deleted and their `project.pbxproj` entries
  removed — but ONLY files still carrying the `// GENERATED by …` marker.
  Hand-written files at old paths are never touched.
