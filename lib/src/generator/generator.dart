...
/// Whether [content] carries the generator's marker header.
///
/// The cleanup step deletes ONLY files that pass this check — a file at a
/// previously-generated path that a human has since replaced is never
/// touched.
bool isGeneratedContent(String content) =>
    content.startsWith('// GENERATED by `dart run '
        'flutter_assistant_intents:generate`');
...
String generateDart(GeneratorConfig config) {
  ...
  buffer
    ...
    ..writeln(
      'const AndroidShortcutsConfig generatedAndroidShortcuts = '
      'AndroidShortcutsConfig(',
    )
    ..writeln(
...
/// Removes every reference to [fileName] from an Xcode project — the exact
/// inverse of [injectIntoPbxproj]. Used by the cleanup step when the Swift
/// output path changes. Returns the input unchanged when nothing matches.
String removeFromPbxproj(String pbxproj, String fileName) {
  final escaped = RegExp.escape(fileName);
  return pbxproj
      .replaceAll(
        RegExp('[ \\t]*[0-9A-F]{24} /\\* $escaped in Sources \\*/ = '
            '\\{isa = PBXBuildFile;[^\\n]*\\n'),
        '',
      )
      .replaceAll(
        RegExp('[ \\t]*[0-9A-F]{24} /\\* $escaped \\*/ = '
