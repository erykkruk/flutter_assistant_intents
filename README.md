...
- Generated files are marked with a `// GENERATED by … — DO NOT EDIT` header
  and are **fully rewritten on every run** — actions removed from the YAML
  disappear from Swift, Siri phrases and Dart automatically; nothing piles up.
- If you change the `output:` paths, the previous files are deleted and
  de-registered from `project.pbxproj` — but **only files still carrying the
  GENERATED marker**. A hand-written file sitting at an old output path is
  never touched (the tool prints a note and leaves it alone).
- **Never hand-edit the `.g.` files** — edits are lost on the next run. Your
  own additional `AppIntent`s belong in separate Swift files (e.g. in
  `AppDelegate.swift` or your own file); the generator neither overwrites nor
  deletes anything outside its two declared outputs.
